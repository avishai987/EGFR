...
HCC827Jun23 = read.table(
  file = "./input_data/osiRoxa_bulk/Jun23/gene_fpkm.xls",
  sep = "\t",
  header = TRUE
)
...
for (i in 1:length(all_signatures)) {
  ...
  mat <- HCC827Jun23[ genes, ] %>% na.omit %>% colMeans() %>% as.data.frame() %>% 
...
```{r}
write.csv(all_scores,"./temp_data/HCC827_JUN23_esrra.csv")
```
