...
```{r}
source("./pipe.R")
input = pipeline$bulk_cell_lines_OSI$input
output = pipeline$bulk_cell_lines_OSI$output
```
...
metadata$condition = metadata$condition %>%   factor(levels = c("Day1_NT","Day1_OSI","Day14_OSI"),ordered = T)
metadata <- metadata[order(metadata$condition), ]

for (cell_line in unique(metadata$cell_type )) {
  cells = rownames(metadata)[metadata$cell_type == cell_line]
dusp6 = cell_lines_data[c("DUSP6","MAPK1"),] %>% select(all_of(cells))
