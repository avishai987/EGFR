...
H1975Oct23 = read.table(
  file = "./input_data/osiRoxa_bulk/Oct23/gene_fpkm.xls",
  sep = "\t",
  header = TRUE
)
...
all_signatures = list(kurppa = kurppa_genes,hadrek = hadrek_genes,FRIDMAN_SENESCENCE_UP = FRIDMAN_SENESCENCE_UP,maynard = maynard_genes,ferroptosis = ferroptosis,esrra_up = esrra_up,esrra_down = esrra_down,hif_targets = hif_targets)
all_scores = data.frame()
for (i in 1:length(all_signatures)) {
  genes = all_signatures[[i]]
  mat <- H1975Oct23[ genes, ] %>% na.omit %>% colMeans() %>% as.data.frame() %>% 
    setnames(names(all_signatures)[[i]]) %>% t() %>% as.data.frame()
  all_scores = rbind(all_scores,mat)
}

all_scores <- t(scale(t(all_scores)))
anno <- as.data.frame(all_scores)
library(ComplexHeatmap) 
library(ggplot2) 
Heatmap(all_scores, cluster_rows = T, cluster_columns = F, column_labels = colnames(anno), name = "fpkm Z-score",row_names_gp = gpar(fontsize = 8)) 
```

```{r}
all_scores = data.frame()
# raw expression esrra
for (pathway in c("esrra_up","esrra_down")) {
  genes = all_signatures[[pathway]]
  mat <- H1975Oct23[ genes, ] %>% na.omit %>% colMeans() %>% as.data.frame() %>% 
    setnames(pathway) %>% t() %>% as.data.frame()
  all_scores = rbind(all_scores,mat)
}
all_scores
```
...
```{r}
write.csv(all_scores,"./temp_data/H1975_OCT23_esrra.csv")
```
