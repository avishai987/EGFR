...
import pickle
f = open('/sci/labs/yotamd/lab_share/avishai.wizel/R_projects/EGFR/Data/cnmf/cnmf_objects/models_2Kvargenes_corrected_cnmf_obj.pckl', 'rb')
...
```{r}
metagenes_mean_compare.2 = function(dataset,time.point_var,prefix = "",patient.ident_var,pre_on = c("OSI","NT"),axis.text.x = 11,test = "t.test", programs = c("Hypoxia","TNFa","Cell_cycle"), with_split = T, without_split = T){
  
  for (metegene in programs) {
    #create data:
    genes_by_tp = FetchData(object = dataset,vars =  c(patient.ident_var,time.point_var,metegene)) %>% filter(treatment != "res")  %>% as.data.frame() #mean expression
...
metagenes_mean_compare.3 = function(dataset,time.point_var,prefix = "",patient.ident_var,pre_on = c("OSI","NT"),axis.text.x = 11,test = "t.test", programs = c("Hypoxia","TNFa","Cell_cycle"), with_split = T, without_split = T){
  ...
  for (metegene in programs) {
    ...
    # plt = ggboxplot(genes_by_tp_forPlot, x = time.point_var, y = "Metagene_mean", color = time.point_var) + #plot
    # plt = ggplot(genes_by_tp, aes(x = orig.ident, y = Metagene_mean, fill=treatment)) +geom_split_violin(scale = 'width')+ylab(metegene)
      plt = BiocompR::ggcraviola(data = genes_by_tp)+ylab(metegene)
