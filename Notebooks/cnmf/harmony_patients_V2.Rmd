...
densify = T #default: T
convert_to_tpm = F #default: T
provideTPM = F  #default: T
theta = 2.7 #default: 2
suffix ="2Kvargenes_logTPM_2-7theta"
...
#Run harmony
```{python}
harmony_res = harmonypy.run_harmony(adata.obsm['X_pca'], adata.obs, 'patient.ident', max_iter_harmony = 15,theta = theta)
```
## Adujst correction
```{python}
...
```{r results='hide'}
DimPlot(lung, reduction = "umap", group.by = "patient.ident")
```

```{r results='hide'}
DimPlot(lung, reduction = "umap", group.by = "time.point")
```
