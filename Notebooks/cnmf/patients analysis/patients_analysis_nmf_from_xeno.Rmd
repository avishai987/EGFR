...
genesets_h = getGmt("./Data/h.all.v2023.2.Hs.symbols.gmt") %>% geneIds()
```
...
colnames(all_metagenes) = c("IFNa_nmf","TNFa_nmf", "hypoxia_nmf","cell_cycle_nmf","EMT_nmf")
...
p = DotPlot.2(object = lung, features =  paste0(names(all_metagenes),"_scaled"),group.by  = 'treatment',scale = F)+
...
p = p[[1]]+xlab("Patient")+ylab(paste( "Program 3 (Hypoxia)\n"))
...
```{r}
p2 = metagenes_violin_compare(dataset = lung,prefix = "Patient",pre_on = c("pre-treatment","on-treatment"),test = "wilcox.test",programs = "hif_targets", return_list = T)[[1]]
p2
```
...
p = DotPlot.2(object = lung, features =   names(gene_list),group.by  = 'treatment',scale = F)+
