...
tiss_subset_tumor2_pri_egfr = subset(tiss_subset_tumor2_pri, subset = driver_gene == "EGFR") %>% subset(subset = histolgy == "Adenocarcinoma") 
```
...
colnames(all_metagenes) = c("IFNa_nmf","TNFa_nmf", "hypoxia_nmf","cell_cycle_nmf","unknown")
...
# NMF programs
```{r}
p = metagenes_violin_combine_patients(dataset = tiss_subset_tumor2_pri_egfr,prefix = "patient",pre_on = c("naive","grouped_pr"),test = "wilcox.test",programs = colnames(all_metagenes), return_list = F)

p
```

```{r}
saveRDS(object = as.grob(p),file = "./Figures/bivona_nmf_split_violin.rds")
```
...
```{r}
p = metagenes_violin_combine_patients(dataset = tiss_subset_tumor2_pri_egfr,prefix = "patient",pre_on = c("naive","grouped_pr"),test = "wilcox.test",programs = names(gene_list), return_list = F)
p
```
```{r}
saveRDS(object = as.grob(p),file = "./Figures/bivona_known_genes_violin.rds")
```
