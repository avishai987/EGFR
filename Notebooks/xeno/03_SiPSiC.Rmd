...
# saveRDS(object = as.grob(as.ggplot(p)),file = params$output_data["xeno_sipsic_heatmap_png"])
png( paste0(params$data_out_dir,params$output_data["xeno_sipsic_heatmap_png"]), width = 8, height = 7,units = "in",res = 250)
p
dev.off()
...
for (patient_name in c("1109","1071")) {
  ...
  p1 = 
   ggplot(data, aes(x = treatment, y = HALLMARK.HYPOXIA)) +
    stat_summary(fun = mean)+
    geom_jitter() +
    geom_text(stat = "summary", aes(label = round(..y.., 2)),nudge_y = 0.2) +
    theme_minimal() +
    labs(title =paste(patient_name, "HALLMARK-HYPOXIA in each treatment"), x = "Treatment", y = "HALLMARK-HYPOXIA")+ 
  ...
  p2 = 
  ggplot(data, aes(x = treatment, y = HALLMARK.TNFA.SIGNALING.VIA.NFKB)) +
    stat_summary(fun = mean)+
    geom_jitter() +
    geom_text(stat = "summary", aes(label = round(..y.., 2)),nudge_y = 0.2) +
    theme_minimal() +
    labs(title = paste(patient_name,"HALLMARK-TNFA-SIGNALING-VIA-NFKB in each treatment"), x = "Treatment", y = "HALLMARK-TNFA-SIGNALING-VIA-NFKB")+ 
    geom_boxplot(aes(group = treatment), fill = "white", width = 0.2, outlier.shape = NA)
