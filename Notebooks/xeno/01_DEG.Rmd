...
input_paths = config.yaml[["./Notebooks/xeno/01_DEG.Rmd"]]$input %>% unlist()
...
genesets = getGmt(input_paths["genesets"]) %>% geneIds()
```
...
saveRDS(object = as.grob(p1),file = paste0(params$data_out_dir,output_paths["xeno_DEG_GSEA_up"]))
saveRDS(object = as.grob(p2),file = paste0(params$data_out_dir,output_paths["xeno_DEG_GSEA_down"]))
```
...
```{r}
saveRDS(object = as.grob(p),file = paste0(params$data_out_dir,output_paths["xeno_DEG_per_patient"]))
```
