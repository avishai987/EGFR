...
p = programs_dotplot(seurat_obj = xeno,treatment_var = "treatment",program_names = names(xeno_cell_usage))
p
```
...
```{r}
svg(params$data_out_dir %s+% "xeno_programs_dotplot.svg",height = 4,width = 8)
p
dev.off()
```
...
program_names =  c("IFNa","TNFa","HIF", "cell cycle")

# for sup:
plt_list = metagenes_violin_compare(dataset = xeno,prefix = "model",pre_on = c("NT","OSI"),test = "wilcox.test",programs = colnames(xeno_cell_usage)[-c(5)],return_list =  T)
...
```{r}
svg(params$data_out_dir %s+% "all_programs_violin_xeno.svg",height = 5,width = 12)
p
dev.off()
```
...
```{r}
svg(params$data_out_dir %s+% "hif_targets_violin_plot.svg")
p2
dev.off()
```
