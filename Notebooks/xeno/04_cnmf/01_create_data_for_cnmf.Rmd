...
```{r}
#run once to install
reticulate::conda_create(envname = "scanpy")

  job::job({
  reticulate::use_condaenv("scanpy")
  print(reticulate::py_config()) # if you see that r-cnmf_dev has not been loaded, restart/terminate r and try again, DO NOT load reticulate before.
  reticulate::conda_install(envname = "scanpy",
              packages = c("scanpy=1.9.8",
                      # "cnmf=1.5.0"
                       "anndata>=0.7.4",
                       "numpy>=1.17.0",
                       "matplotlib>=3.6",
              ...
              channel = c("conda-forge",
                          rep("defaults", 20)))
    job::export("none")
  }, import = NULL, packages = NULL,title = "install scanpy conda env")
...
```{r}
reticulate::use_condaenv("C:/Users/avishaiw.EKMD/AppData/Local/r-miniconda/envs/scanpy",required = T)
