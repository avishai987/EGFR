...
source("./pipe.R")
input = pipeline$xenografts_deg$input
output = pipeline$xenografts_deg$output
```
...
hallmark_genesets = getGmt(input$genesets) %>% geneIds()
hif_targets = scan(input$hif_targets, character(), quote = "")
# add hif targets to genesets
hallmark_genesets[["hif_targets"]] = hif_targets
...
hyp_obj <-hypeR_fgsea(signature = ranked_vec,genesets = (hallmark_genesets),up_only = F)
...
svg(paste0(params$data_out_dir,"DEG_GSEA_xeno.svg"), width = 8, height = 5)
p1
dev.off()
